...
function checkNewDeps(cwd: string, stagedFiles: string[]): ReviewFinding {
  ...
  if (!stagedFiles.some(f => depFiles.includes(f))) {
...
function scanLineForSecrets(line: string, file: string, lineNum: number): string | null {
  if (/your[_-]?(?:key|token|secret|value)|changeme|placeholder|xxx+|TODO/i.test(line)) return null;
  for (const pattern of SECRET_PATTERNS) {
    if (pattern.regex.test(line)) return `${file}:${lineNum} -- ${pattern.name}`;
  }
  return null;
}
...
function checkSecrets(cwd: string, stagedFiles: string[]): ReviewFinding {
  ...
  for (const file of stagedFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const found = scanLineForSecrets(lines[i], file, i + 1);
...
function checkDebugStatements(diff: string): ReviewFinding {
  const debugLines = diff.split('\n')
    .filter(l => l.startsWith('+') && !l.startsWith('+++'))
    .filter(l => /console\.(log|debug|warn)\s*\(/.test(l) || /\bprint\s*\(/.test(l));
...
function checkFileSize(cwd: string, stagedFiles: string[]): ReviewFinding {
  ...
  if (largeFiles.length > 0) {
    return {
      check: 'File size',
      status: 'warn',
      message: `${largeFiles.length} file(s) over ${DEFAULT_MAX_FILE_LINES} lines`,
